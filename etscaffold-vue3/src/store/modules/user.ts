...
import { ROLES_KEY, TOKEN_KEY, ID_TOKEN_KEY, USER_INFO_KEY, LOGIN_INFO_KEY, DB_DICT_DATA_KEY, TENANT_ID } from '/@/enums/cacheEnum';
import { getAuthCache, setAuthCache, removeAuthCache } from '/@/utils/auth';
import { GetUserInfoModel, LoginParams, ThirdLoginParams, RevokeIntrospectTokenParams } from '/@/api/sys/model/userModel';
...
export const useUserStore = defineStore({
  ...
  getters: {
    ...
    getId_token(): string {
      return this.id_token || getAuthCache<string>(ID_TOKEN_KEY);
    },
    getAllDictItems(): [] {
  ...
  actions: {
    ...
    setId_token(info: string | undefined) {
      this.token = info ? info : ''; // for null or undefined value
      setAuthCache(ID_TOKEN_KEY, info);
    },
    ...
    async logout(goLogin = false) {
      ...
      const { VITE_OAUTH2_CLIENTID } = import.meta.env;
      const logoutParams : RevokeIntrospectTokenParams = {
        client_id : VITE_OAUTH2_CLIENTID,
        token: this.getToken
      }
      let id_token = this.getId_token
      this.setToken('');
      this.setId_token('');
      setAuthCache(TOKEN_KEY, null);
      setAuthCache(ID_TOKEN_KEY, null);
      this.setSessionTimeout(false);
      ...
      goLogin && (useSso().doOauth2SSOLogOut(logoutParams, id_token));
    },
    /**
     * 登录事件
     */
