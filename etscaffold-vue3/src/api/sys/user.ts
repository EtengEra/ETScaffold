...
import { LoginParams, LoginResultModel, GetUserInfoModel, Oauth2LoginParams, RevokeIntrospectTokenParams } from './model/userModel';
...
enum Api {
  Login = '/scaffold/callEtCloudLogin',
  phoneLogin = '/sys/phoneLogin',
  Logout = '/sys/logout',
  GetUserInfo = '/scaffold/callEtCloudUserInfo',
  // 获取系统权限
  // 1、查询用户拥有的按钮/表单访问权限
  // 2、所有权限
  // 3、系统安全模式
  ...
  //oauth2 oss 获取code 
  getOauth2OssCode = '/oauth2/getCode',
  //通过code 获取token
  getOauth2Token = '/auth/token',
  //登出功能 注销token
  oauth2SSOLoginOut= '/auth/revoke',
}

/**
 * 登出功能 注销token
 */
export function oauth2SSOLoginOut(params : RevokeIntrospectTokenParams){
  return defHttp.post({ url: Api.oauth2SSOLoginOut, params });
}

/**
 * 通过code 获取 认证中心的 token
 */
export function getOauth2Token(params : Oauth2LoginParams) {
  return defHttp.post(
    { 
      url: Api.getOauth2Token,
      params
    }, 
    {
      //隐藏后端返回的成功提示信息
 ...
 * @returns 
 */
export function getOauth2OssCode(code) {
  let url = Api.getOauth2OssCode + `/${code}`;
